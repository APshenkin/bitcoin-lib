...
import com.apshenkin.verge.Crypto._
import com.apshenkin.verge.Protocol._
...
class TransactionSpec extends FunSuite with Matchers {
  test("create and sign transaction") {
    ...
    writeUInt32(1595234975, out) //time
    ...
    assert(res === hex"")
  }
  test("read and write transactions") {
    val hex = hex""
    ...
    val tx1 = Transaction(
      ...
      txIn = List(
        TxIn(
          ...
          signatureScript = previousTx.txOut.head.publicKeyScript,
  ...
  test("create and verify pay2pk transactions with 1)put/1 output using helper method") {
    ...
    // step #2: sign the tx
    val sig = Transaction.signInput(tx1, 0, previousTx.txOut.head.publicKeyScript, SIGHASH_ALL, 0 sat, SigVersion.SIGVERSION_BASE, privateKey)
  ...
  test("create and verify sign pay2pk transactions with multiple)puts and outputs") {
    ...
    val sig1 = Transaction.signInput(tx, 0, previousTx.head.txOut.head.publicKeyScript, SIGHASH_ALL, 0 sat, SigVersion.SIGVERSION_BASE, keys.head)
    val sig2 = Transaction.signInput(tx, 1, previousTx(1).txOut.head.publicKeyScript, SIGHASH_ALL, 0 sat, SigVersion.SIGVERSION_BASE, keys(1))
    val tx1 = tx
  ...
  test("create and sign p2sh transactions") {
    ...
    // and sign it
    val sig = Transaction.signInput(tx, 0, previousTx.txOut.head.publicKeyScript, SIGHASH_ALL, 0 sat, SigVersion.SIGVERSION_BASE, privateKey)
  ...
  test("sign a 3-to-2 transaction with helper method") {
    ...
    val tx = Transaction(
      ...
      time = System.currentTimeMillis / 1000,
      txIn = List(
        TxIn(OutPoint(previousTx.head, 1), ByteVector.empty, 0xffffffffL),
    ...
    val sig1 = Transaction.signInput(tx, 0, previousTx.head.txOut(1).publicKeyScript, SIGHASH_ALL, 0 sat, SigVersion.SIGVERSION_BASE, keys.head)
    val sig2 = Transaction.signInput(tx, 1, previousTx(1).txOut.head.publicKeyScript, SIGHASH_ALL, 0 sat, SigVersion.SIGVERSION_BASE, keys(1))
    val sig3 = Transaction.signInput(tx, 2, previousTx(2).txOut.head.publicKeyScript, SIGHASH_ALL, 0 sat, SigVersion.SIGVERSION_BASE, keys(2))
