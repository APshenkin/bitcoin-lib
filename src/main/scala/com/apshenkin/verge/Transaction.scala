...
import com.apshenkin.verge.Script.Runner
...
object OutPoint extends BtcSerializer[OutPoint] {
  ...
  def isCoinbase(input: OutPoint): Boolean = input.index == 0xffffffffL && input.hash == ByteVector32.Zeroes
...
object TxIn extends BtcSerializer[TxIn] {
  ...
  val SEQUENCE_LOCKTIME_DISABLE_FLAG: Long = 1L << 31

  /* If CTxIn::nSequence encodes a relative lock-time and this flag
   * is set, the relative lock-time has units of 512 seconds,
   * otherwise it specifies blocks with a granularity of 1. */
  val SEQUENCE_LOCKTIME_TYPE_FLAG: Long = 1L << 22
...
object Transaction extends BtcSerializer[Transaction] {
  ...
  override def validate(input: Transaction): Unit = {
    ...
    if (Transaction.isCoinbase(input)) {
      require(input.txIn.head.signatureScript.size >= 2, "coinbase script size")
      require(input.txIn.head.signatureScript.size <= 100, "coinbase script size")
    } else {
